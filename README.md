...
4. Добавить сохранение через Dapper и Docker Compose с PostgreSQL и pgAdmin — выполнено.
5. Проверить оба входных примера и некорректные запросы, сохранить фактические
   ответы в `json_result_1.txt` и `json_result_2.txt`, описать запуск — выполнено.
...
Проверки добавляют тестовые строки с уникальным marker текущего запуска.
...
Docker Compose проверен полностью: `docker compose up -d --wait` поднимает API,
PostgreSQL 18 и pgAdmin в статусе healthy. API собирается внутри SDK-контейнера
при старте (`Build succeeded` в логах), Swagger JSON доступен на
`/api/swagger/v1/swagger.json`, Swagger UI открывается через `/api/swagger`,
pgAdmin отвечает на `localhost:8080`, сервер `TestJob PostgreSQL 18` импортирован
и использует `.pgpass` с правами `600`. После `docker compose down` и повторного
`up -d --wait` том PostgreSQL сохранил 256 строк тестовых данных.
